...
This makes performance in the more common cloud deployment largely untested.
...
It normalizes performance on resource usage meaning that a protocol with a higher measurement generates more throughput at the same resource usage than a protocol with a lower measurement.
It ignores load balancing and factores in only the resources actually used.
...
QuePaxa and RACS aim not to increase throughput by large amounts, but to increase resilience.
They should perform comparatively to Multi-Paxos and Raft under normal network conditions. Under bad network conditions like misconfigurations or a DoS-attack, they aim to retain much more throughput than Multi-Paxos or Raft.
QuePaxa achieves this by reworking the leader change logic from relying on conservative timeouts to ensure liveness, to relying on short hedging delays and by implementing a randomized asynchronous core.
RACS is an even more modern consensus protocol with partially the same authors as QuePaxa.
RACS goes a step further and wants to have good performance in both good and bad network conditions, while being easy to integrate into an existing software stack by being similar to Raft.
...
RACS is even more clear with the focus on latency versus throughput in its testing, also not showing a single efficiency graph.
...
The paper has a large focus on throughput. While occasionally mentioning it, latency is mostly ignored in this paper. Latency however, is one of the biggest focuses of other works and important to many applications.
An application sensitive to latency would gain nothing when evaluating potential protocol choices on their throughput per CPU\% alone. It might even perform worse compared to a less efficient protocol.
If the measure is used to improve an existing protocol, only evaluating throughput might drastically increase latency without getting noticed leading to an overall worse protocol.
