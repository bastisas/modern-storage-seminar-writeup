...
%- modern consensus protocols  increase throughput by eliminating bottlenecks, lowering efficiency in the process.
...
%https://discovery.ucl.ac.uk/id/eprint/10181480/1/quepaxa.pdf?utm_source=chatgpt.com
%https://arxiv.org/pdf/2404.04183

%RACS does not give cpu usage data and seems to test its intances in isolation not with mutliple multi-paxos instances 

%they focus on latency which my paper does not mention.

%main takeaway for at least these two papers from partially the same authors semms to be EPaxos' latency characteristics.

There are many protocols that have been developed since this one was published, that focus on cloud consensus. Two of them are QuePaxa[1] and RACS[2]. %ADD reference
...
QuePaxa should perform comparatively to Multi-Paxos and Raft under normal network conditions. Under bad network conditions like a DoS-attack or misconfigurations, QuePaxa aims to retain much more throughput than Multi-Paxos or Raft.
...
It does, however, also not only focus on increasing throughput leaving efficiency behind as it measures performance relative to latency.
...
The paper uses illustration very well to explain its topics. The two figures of Multi-Paxos' bad load balancing and of task packing in a cloud scenario help understand the concepts well.
The graphs never show changing values over time, instead only mostly constant values are represented. 
Only mentioning these numbers in text, however, would greatly impact the understandability of the paper, which is why the inclusion of these graphs and the selection of which to include, is very important and done well.
...
Even in the tests they do run, they compare two protocols on an undescriptive key-value store, missing even the chance to compare Multi-Paxos and Raft or to use real world usen cases and datasets.

On top of a limited scope, the paper repeats itself many times. % anpassen weil eine introduction normal ist.
...
%Core idea: efficiency is key to performance in resource shared environments.
%- some way of using the metric outside of replication protocols:
%- Gather data: Big chart with Multi-Paxos at 100\% and all other protocols at their relative efficiency
